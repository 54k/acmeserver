...
public final class QuadTree<T> {
    ...
    public QuadTree(int height, int width, int nodeHeight, int nodeWidth, ContextFactory<T> contextFactory) {
        this.width = width;
        this.height = height;
    ...
    private void initNodes() {
        for (int i = 0; i < width; i += nodeWidth) {
            for (int j = 0; j < height; j += nodeHeight) {
                ...
                nodesByIndex.put(regionIndex, new QuadTreeNode<>(contextFactory.getContext()));
            }
        }
    ...
    /**
     * Creates context instance to be associate with {@link QuadTreeNode}
     */
    public static interface ContextFactory<T> {
